...
def login(payload: LoginRequest, request: Request):
    ...
    for teacher in teachers:
        if (
            secrets.compare_digest(payload.username, teacher["username"])
            and secrets.compare_digest(payload.password, teacher["password"])
        ):
            request.session["teacher_username"] = teacher["username"]
            return {"message": f"Logged in as {teacher['username']}"}
